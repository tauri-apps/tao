...
fn is_numpad_specific(vk: u32) -> bool {
  match vk {
    _ if vk == VK_NUMPAD0 => true,
    _ if vk == VK_NUMPAD1 => true,
    _ if vk == VK_NUMPAD2 => true,
    _ if vk == VK_NUMPAD3 => true,
    _ if vk == VK_NUMPAD4 => true,
    _ if vk == VK_NUMPAD5 => true,
    _ if vk == VK_NUMPAD6 => true,
    _ if vk == VK_NUMPAD7 => true,
    _ if vk == VK_NUMPAD8 => true,
    _ if vk == VK_NUMPAD9 => true,
    _ if vk == VK_ADD => true,
    _ if vk == VK_SUBTRACT => true,
    _ if vk == VK_DIVIDE => true,
    _ if vk == VK_DECIMAL => true,
    _ if vk == VK_SEPARATOR => true,
    _ => false,
  }
}
...
fn vkey_to_non_char_key(
  ...
  match vkey {
    _ if vkey == VK_LBUTTON => Key::Unidentified(NativeKeyCode::Unidentified), // Mouse
    _ if vkey == VK_RBUTTON => Key::Unidentified(NativeKeyCode::Unidentified), // Mouse

    // I don't think this can be represented with a Key
    _ if vkey == VK_CANCEL => Key::Unidentified(native_code),

    _ if vkey == VK_MBUTTON => Key::Unidentified(NativeKeyCode::Unidentified), // Mouse
    _ if vkey == VK_XBUTTON1 => Key::Unidentified(NativeKeyCode::Unidentified), // Mouse
    _ if vkey == VK_XBUTTON2 => Key::Unidentified(NativeKeyCode::Unidentified), // Mouse
    _ if vkey == VK_BACK => Key::Backspace,
    _ if vkey == VK_TAB => Key::Tab,
    _ if vkey == VK_CLEAR => Key::Clear,
    _ if vkey == VK_RETURN => Key::Enter,
    _ if vkey == VK_SHIFT => Key::Shift,
    _ if vkey == VK_CONTROL => Key::Control,
    _ if vkey == VK_MENU => Key::Alt,
    _ if vkey == VK_PAUSE => Key::Pause,
    _ if vkey == VK_CAPITAL => Key::CapsLock,

    //_ if vkey == VK_HANGEUL => Key::HangulMode, // Deprecated in favour of VK_HANGUL
    ...
    _ if vkey == VK_HANGUL && is_korean => Key::HangulMode,
    _ if vkey == VK_KANA && is_japanese => Key::KanaMode,

    _ if vkey == VK_JUNJA => Key::JunjaMode,
    _ if vkey == VK_FINAL => Key::FinalMode,

    // VK_HANJA and VK_KANJI are defined as the same constant, therefore
    // we use appropriate conditions to differentate between them
    _ if vkey == VK_HANJA && is_korean => Key::HanjaMode,
    _ if vkey == VK_KANJI && is_japanese => Key::KanjiMode,

    _ if vkey == VK_ESCAPE => Key::Escape,
    _ if vkey == VK_CONVERT => Key::Convert,
    _ if vkey == VK_NONCONVERT => Key::NonConvert,
    _ if vkey == VK_ACCEPT => Key::Accept,
    _ if vkey == VK_MODECHANGE => Key::ModeChange,
    _ if vkey == VK_SPACE => Key::Space,
    _ if vkey == VK_PRIOR => Key::PageUp,
    _ if vkey == VK_NEXT => Key::PageDown,
    _ if vkey == VK_END => Key::End,
    _ if vkey == VK_HOME => Key::Home,
    _ if vkey == VK_LEFT => Key::ArrowLeft,
    _ if vkey == VK_UP => Key::ArrowUp,
    _ if vkey == VK_RIGHT => Key::ArrowRight,
    _ if vkey == VK_DOWN => Key::ArrowDown,
    _ if vkey == VK_SELECT => Key::Select,
    _ if vkey == VK_PRINT => Key::Print,
    _ if vkey == VK_EXECUTE => Key::Execute,
    _ if vkey == VK_SNAPSHOT => Key::PrintScreen,
    _ if vkey == VK_INSERT => Key::Insert,
    _ if vkey == VK_DELETE => Key::Delete,
    _ if vkey == VK_HELP => Key::Help,
    _ if vkey == VK_LWIN => Key::Super,
    _ if vkey == VK_RWIN => Key::Super,
    _ if vkey == VK_APPS => Key::ContextMenu,
    _ if vkey == VK_SLEEP => Key::Standby,

    // Numpad keys produce characters
    _ if vkey == VK_NUMPAD0 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD1 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD2 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD3 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD4 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD5 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD6 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD7 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD8 => Key::Unidentified(native_code),
    _ if vkey == VK_NUMPAD9 => Key::Unidentified(native_code),
    _ if vkey == VK_MULTIPLY => Key::Unidentified(native_code),
    _ if vkey == VK_ADD => Key::Unidentified(native_code),
    _ if vkey == VK_SEPARATOR => Key::Unidentified(native_code),
    _ if vkey == VK_SUBTRACT => Key::Unidentified(native_code),
    _ if vkey == VK_DECIMAL => Key::Unidentified(native_code),
    _ if vkey == VK_DIVIDE => Key::Unidentified(native_code),

    _ if vkey == VK_F1 => Key::F1,
    _ if vkey == VK_F2 => Key::F2,
    _ if vkey == VK_F3 => Key::F3,
    _ if vkey == VK_F4 => Key::F4,
    _ if vkey == VK_F5 => Key::F5,
    _ if vkey == VK_F6 => Key::F6,
    _ if vkey == VK_F7 => Key::F7,
    _ if vkey == VK_F8 => Key::F8,
    _ if vkey == VK_F9 => Key::F9,
    _ if vkey == VK_F10 => Key::F10,
    _ if vkey == VK_F11 => Key::F11,
    _ if vkey == VK_F12 => Key::F12,
    _ if vkey == VK_F13 => Key::F13,
    _ if vkey == VK_F14 => Key::F14,
    _ if vkey == VK_F15 => Key::F15,
    _ if vkey == VK_F16 => Key::F16,
    _ if vkey == VK_F17 => Key::F17,
    _ if vkey == VK_F18 => Key::F18,
    _ if vkey == VK_F19 => Key::F19,
    _ if vkey == VK_F20 => Key::F20,
    _ if vkey == VK_F21 => Key::F21,
    _ if vkey == VK_F22 => Key::F22,
    _ if vkey == VK_F23 => Key::F23,
    _ if vkey == VK_F24 => Key::F24,
    _ if vkey == VK_NAVIGATION_VIEW => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_MENU => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_UP => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_DOWN => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_LEFT => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_RIGHT => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_ACCEPT => Key::Unidentified(native_code),
    _ if vkey == VK_NAVIGATION_CANCEL => Key::Unidentified(native_code),
    _ if vkey == VK_NUMLOCK => Key::NumLock,
    _ if vkey == VK_SCROLL => Key::ScrollLock,
    _ if vkey == VK_OEM_NEC_EQUAL => Key::Unidentified(native_code),
    //_ if vkey == VK_OEM_FJ_JISHO => Key::Unidentified(native_code), // Conflicts with `VK_OEM_NEC_EQUAL`
    _ if vkey == VK_OEM_FJ_MASSHOU => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_FJ_TOUROKU => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_FJ_LOYA => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_FJ_ROYA => Key::Unidentified(native_code),
    _ if vkey == VK_LSHIFT => Key::Shift,
    _ if vkey == VK_RSHIFT => Key::Shift,
    _ if vkey == VK_LCONTROL => Key::Control,
    _ if vkey == VK_RCONTROL => Key::Control,
    _ if vkey == VK_LMENU => Key::Alt,
    _ if vkey == VK_RMENU => {
      if has_alt_graph {
        Key::AltGraph
      } else {
    ...
    _ if vkey == VK_BROWSER_BACK => Key::BrowserBack,
    _ if vkey == VK_BROWSER_FORWARD => Key::BrowserForward,
    _ if vkey == VK_BROWSER_REFRESH => Key::BrowserRefresh,
    _ if vkey == VK_BROWSER_STOP => Key::BrowserStop,
    _ if vkey == VK_BROWSER_SEARCH => Key::BrowserSearch,
    _ if vkey == VK_BROWSER_FAVORITES => Key::BrowserFavorites,
    _ if vkey == VK_BROWSER_HOME => Key::BrowserHome,
    _ if vkey == VK_VOLUME_MUTE => Key::AudioVolumeMute,
    _ if vkey == VK_VOLUME_DOWN => Key::AudioVolumeDown,
    _ if vkey == VK_VOLUME_UP => Key::AudioVolumeUp,
    _ if vkey == VK_MEDIA_NEXT_TRACK => Key::MediaTrackNext,
    _ if vkey == VK_MEDIA_PREV_TRACK => Key::MediaTrackPrevious,
    _ if vkey == VK_MEDIA_STOP => Key::MediaStop,
    _ if vkey == VK_MEDIA_PLAY_PAUSE => Key::MediaPlayPause,
    _ if vkey == VK_LAUNCH_MAIL => Key::LaunchMail,
    _ if vkey == VK_LAUNCH_MEDIA_SELECT => Key::LaunchMediaPlayer,
    _ if vkey == VK_LAUNCH_APP1 => Key::LaunchApplication1,
    _ if vkey == VK_LAUNCH_APP2 => Key::LaunchApplication2,

    // This function only converts "non-printable"
    _ if vkey == VK_OEM_1 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_PLUS => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_COMMA => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_MINUS => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_PERIOD => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_2 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_3 => Key::Unidentified(native_code),

    _ if vkey == VK_GAMEPAD_A => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_B => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_X => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_Y => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_SHOULDER => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_SHOULDER => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_TRIGGER => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_TRIGGER => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_DPAD_UP => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_DPAD_DOWN => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_DPAD_LEFT => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_DPAD_RIGHT => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_MENU => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_VIEW => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_THUMBSTICK_BUTTON => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_THUMBSTICK_BUTTON => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_THUMBSTICK_UP => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_THUMBSTICK_DOWN => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_THUMBSTICK_RIGHT => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_LEFT_THUMBSTICK_LEFT => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_THUMBSTICK_UP => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_THUMBSTICK_DOWN => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_THUMBSTICK_RIGHT => Key::Unidentified(native_code),
    _ if vkey == VK_GAMEPAD_RIGHT_THUMBSTICK_LEFT => Key::Unidentified(native_code),

    // This function only converts "non-printable"
    _ if vkey == VK_OEM_4 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_5 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_6 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_7 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_8 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_AX => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_102 => Key::Unidentified(native_code),

    _ if vkey == VK_ICO_HELP => Key::Unidentified(native_code),
    _ if vkey == VK_ICO_00 => Key::Unidentified(native_code),

    _ if vkey == VK_PROCESSKEY => Key::Process,

    _ if vkey == VK_ICO_CLEAR => Key::Unidentified(native_code),
    _ if vkey == VK_PACKET => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_RESET => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_JUMP => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_PA1 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_PA2 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_PA3 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_WSCTRL => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_CUSEL => Key::Unidentified(native_code),

    _ if vkey == VK_OEM_ATTN => Key::Attn,
    _ if vkey == VK_OEM_FINISH => {
      if is_japanese {
        Key::Katakana
      } else {
    ...
    _ if vkey == VK_OEM_COPY => Key::Copy,
    _ if vkey == VK_OEM_AUTO => Key::Hankaku,
    _ if vkey == VK_OEM_ENLW => Key::Zenkaku,
    _ if vkey == VK_OEM_BACKTAB => Key::Romaji,
    _ if vkey == VK_ATTN => Key::KanaMode,
    _ if vkey == VK_CRSEL => Key::CrSel,
    _ if vkey == VK_EXSEL => Key::ExSel,
    _ if vkey == VK_EREOF => Key::EraseEof,
    _ if vkey == VK_PLAY => Key::Play,
    _ if vkey == VK_ZOOM => Key::ZoomToggle,
    _ if vkey == VK_NONAME => Key::Unidentified(native_code),
    _ if vkey == VK_PA1 => Key::Unidentified(native_code),
    _ if vkey == VK_OEM_CLEAR => Key::Clear,
